<?php
include("header.php");
?>
<section class="card-list">
  <!-- <div class="introduce-cardlist">
    Découvrez l'équipe qui vous fera voyager encore plus loin que vos rêves !!!
  </div> -->
  <article class="card">
    <div class="profile-sidebar">
      <img class="profile-image" src="images/Léa.jpg" alt="">
    </div>
    <div class="profile-main">
      <h3 class="profile-name">Léa M</h3>
      <p class="profile-position">Web Developer</p>
      <p class="profile-body">
      <span>Curieuse et <b>baroudeuse</b> dans l'âme, Léa vous déniche les meilleures spots pour vivre votre Trip dans les conditions les plus Wild...<br><br> Elle se fera un plaisir de répondre à vos questions (elle adore ça !)</span>
      </p>
    </div>
  </article>

  <article class="card">
    <div class="profile-sidebar">
      <img class="profile-image" src="images/lionel.jpg" alt="">
    </div>
    <div class="profile-main">
      <h3 class="profile-name">Lionel G</h3>
      <p class="profile-position">Web Developer</p>
      <p class="profile-body">
      <span>Lionel, le <b>sage</b> de notre team Very Wild Trip. Ses plus longues destinations : la Thaïlande, l'île Maurice et la Côte d'Ivoire.<br><br> Bienvenue à bord !</span>
      </p>
    </div>
  </article>

  <article class="card">
    <div class="profile-sidebar">
      <img class="profile-image" src="images/Manu.jpg" alt="">
    </div>
    <div class="profile-main">
      <h3 class="profile-name">Emmanuel R</h3>
      <p class="profile-position">Web Developer</p>
      <p class="profile-body">
      <span><b>"Gamer"</b>invétéré depuis sa tendre enfance.
      "Manu" comme préfère le nommer l'équipe, aime passer son temps (quand il n'est pas en train de jouer...) à écrires des lignes de codes ou faire des blagues à ses collègues de travail.<br>
      FORZA VIOLA !</span>
      </p>
    </div>
  </article>

  <article class="card">
    <div class="profile-sidebar">
      <img class="profile-image" src="images/Quentin.jpg" alt="">
    </div>
    <div class="profile-main">
      <h3 class="profile-name">Quentin B</h3>
      <p class="profile-position">Web Developer</p>
      <p class="profile-body">
      <span>Quand il ne joue pas le rôle de <b>"Papa poule"</b>, Quentin vogue à travers les lignes de code. La musique agit telle la feuille de route de son périple, tandis que la patience, la créativité et l'humour en sont le carburant !</span>
      </p>
    </div>
  </article>


</section>




<?php
include("footer.php");
?>


<!-- styleCss about -->
<!-- 
.descript:first-letter{
  font-size: 100px;
  display: block;
  float: left;
  line-height: 0.5;
  margin: 15px 15px 10px 0;
}
.descript{
    font-size: 1rem;
    width:40%;
    margin: auto;
    font-family: "Lato", sans-serif;
} -->